    ...
    if (backendEnabled) {
        return {
            ...
            retentionHint: tool.schema
                .string()
                .describe(
                    "Structured Retain checklist for the backend summaries: user intent, verification evidence, important changes/decisions/paths/errors, next steps, and do-not-overclaim constraints",
                ),
            content: tool.schema
                .array(
                    tool.schema.object({
